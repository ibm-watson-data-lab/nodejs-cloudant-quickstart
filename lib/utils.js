...
var objectify = function(d, field) {
  ...
  if (field && field.length > 1) {
    ...
    console.log('objectify', val);
    d.value = val;
    return d;
  } else {
    d.value = enhance(d.value);
  }
  console.log('objectify unchanged');
  return d;
};


var simplify = function(data, field) {
  console.log('simplifying', data, 'fields', field)
  // [ { key:null, value: 456 }] => 456
  if (isArray(data) && data.length === 1 && data[0].key === null) {
    data[0] = objectify(data[0], field);
    return data[0].value;
  }
